...
const STORAGE_KEY = "ethioairlines.oauth.v1";
...
export type UseEthioAirlinesTokenResult = {
   ...
   * Pass as `Authorization: Bearer ${token}` to the Ethio Airlines API routes.
   */
  ensureValidToken: () => Promise<string | null>;
};

/**
 * Fetches and caches the OAuth token for the Ethio Airlines checkout
 * (via /api/ethioairlines/token).
 * Uses sessionStorage to avoid redundant calls on refresh while still valid.
 */
export function useEthioAirlinesToken(options?: {
  enabled?: boolean;
}): UseEthioAirlinesTokenResult {
  ...
  const refreshToken = useCallback(async (): Promise<string | null> => {
    ...
    setError(null);
    try {
      const res = await fetch("/api/ethioairlines/token", {
        method: "GET",
        cache: "no-store",
      });
