...
// import { useMemo, useRef, useState, useEffect } from "react"
// import { Download, Maximize2, Send } from "lucide-react"
// import * as htmlToImage from "html-to-image"
// import { useToast } from "@/hooks/use-toast"
...
//   const embossedShadow = "-1px -1px 0 rgba(255,255,255,0.75), 1px 1px 0 rgba(0,0,0,0.35)"
//   function embossedTextStyle(elementX: number, elementY: number): React.CSSProperties {
//     if (bgMode === "image" && bgImage) {
//       return {
//         color: "transparent",
//         WebkitTextFillColor: "transparent",
...
//       const link = document.createElement("a")
//       link.href = url
//       link.download = `virtual-card-${exportScale}x-${new Date().toISOString().slice(0, 10)}.png`
//       document.body.appendChild(link)
//       link.click()
//       link.remove()
//       URL.revokeObjectURL(url)
...
//   const addGradientStop = () => {
//     const lastPos = Math.min(100, Math.max(0, (gradient.stops[gradient.stops.length - 1]?.pos ?? 100) - 0))
//     setGradient((g) => ({
//       ...g,
//       stops: [
//         ...g.stops,
...
//       const raw = localStorage.getItem(STORAGE_KEY)
//       if (!raw) {
//         if (showToast) toast({ title: "No saved design found", variant: "destructive" })
//         return
//       }
//       const parsed = JSON.parse(raw) as DesignSnapshot
//       applySnapshot(parsed)
...
//     mutationFn: (data: DesignSnapshot) => createCard(data),
//     onSuccess: () => {
//       toast({
...
//     mutationFn: (data: DesignSnapshot) => (cardId ? updateCard(cardId, data) : Promise.reject("No card ID")),
//     onSuccess: () => {
//       toast({
//         title: "Design Updated",
//         description: "The card design has been successfully updated.",
//       })
//       if (onSuccess) {
//         onSuccess()
//       } else {
...
//       toast({
//         title: "Update Failed",
//         description: "Failed to update the design. Please try again.",
//         variant: "destructive",
//       })
//     },
//   })
...
//                   borderRadius: 24,
//                 }}
//                 aria-label="Card design canvas fullscreen"
//               ></div>
//             </div>
//           </div>
//         </div>
...
//       {/* Tools Panel */}
//       <section className="space-y-4 md:sticky md:top-4 md:self-start md:max-h-[calc(100dvh-2rem)] overflow-y-auto pr-1">
//         {/* Quick Actions Card */}
//         <Card className="border-0 bg-gradient-to-br from-blue-50 to-blue-50/50 dark:from-blue-950/30 dark:to-blue-900/20 shadow-sm">
//           <CardHeader className="pb-3">
...
//                 Save
//               </Button>
//               <Button size="sm" variant="outline" onClick={() => loadFromLocalStorage(true)} className="flex-1">
//                 Load
//               </Button>
//               <Button size="sm" variant="ghost" onClick={resetDesign} className="flex-1">
//                 Reset
//               </Button>
//             </div>
...
//                   <p className="text-xs text-muted-foreground">
//                     {isEditMode ? "Edit card design" : "Live card design"}
//                   </p>
//                 </div>
...
//                   </Button>
//                   <Button size="sm" onClick={exportAsPng} disabled={exporting} className="gap-2">
...
//               </div>
//             </CardHeader>
//             <CardContent className="pt-8 pb-8 px-6 flex items-center justify-center min-h-[500px]"></CardContent>
//           </Card>
//         </div>
//       </section>
//     </div>
...
export default function CardDesigner({ cardId, initialDesign, onSuccess }: CardDesignerProps) {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      // console.log("checking out id", data.id)
    },
    onError: (error) => {
      toast({
        title: "Creation Failed",
              ...
              <div className="flex items-center justify-center">
                ...
                >
                  ...
                  {layers.map((layer, index) => {
                    ...
                    if (normalizedType === "fixed-expiry") {
                      const x = layer.x ?? 0;
                      const y = layer.y ?? 0;
                      return (
                        <div
